...
import auto.server.core.AutoServer;
...
public class AssetModel extends TestCaseNode {
    ...
    boolean hasVisitWaitForProcess = false;
            ...
            if (jsonArray != null) {
                ...
                for (TestCaseNode caseNode : caseNodes) {
                    ...
                    if (caseNode.arg.startsWith("$")) {
                        caseNode.arg = GlobalContent.getConfig(caseNode.arg, testCase.deviceOS,testCase.batchModel.batch_owner);
                    }
    ...
    public int isContainValue(JSONArray jsonArray, ArrayList<String> keys) {
        ...
        for (int index = 0; index < count; index++) {
            ...
            if (keys.indexOf(className) >= 0) {
                int i  = keys.indexOf(className);
                if(hasVisitWaitForProcess){
                    TestCaseNode caseNode = caseNodes.get(i);
                    if (!caseNode.args[1].trim().replace("goto:", "").equals("waitProcess")) {
                        return i;
                    }
                }else{
